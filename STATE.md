...
|---|---|---|---|---|---|---|
| A1 | CMake 骨架 + 目录结构 + 编译选项(`-mcpu=apple-m4 -O3`) | 可构建空工程 + tests 框架 | 未分配 | TODO | — | 含 .gsv 头部解析单测 |
| A2 | `.gsv` 读取器（mmap、目录解析、64B 对齐校验） | `src/runtime/gsv_loader` + 单测 | 未分配 | TODO | — | 对照 tools/convert.py docstring 规范 |
| A3 | NEON 内核层 v1: GEMV(fp16存储+fp32累加)/rmsnorm/rope/silu/softmax | `src/kern/*` + 数值单测 | 未分配 | TODO | — | 单测直接对 golden 层快照 |
| A4 | Accelerate 封装: sgemm/hgemm 薄包装 + 线程池(QoS 分簇) | `src/kern/accel*` | 未分配 | TODO | — | |
